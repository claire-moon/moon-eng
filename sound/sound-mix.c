...
void mixPlayTone(int ch, int waveType, float freq, float vol) {
  ...
  channels[ch].attackRate   = 200;
  channels[ch].decayRate    = 30;
  channels[ch].sustainLevel = 32000;
  channels[ch].releaseRate  = 10;
...
void mixUpdate() {
  ...
  for (i = startIdx; i < startIdx + halfSize; i++) {
	...
	rawSample = ((channels[c].phaseAcc >> 8) & 0xFF) - 128.0;
	break;
	
      case WAVE_SQUARE:
	
	rawSample = (channels[c].phaseAcc & 0x8000) ? 127.0 : -128.0;
	break;
	...
	rawSample = (float)(rand() % 256) - 128.0;
	break;
